...
if __name__ == "__main__":
    ...
    # 比例控制进行调整,相关参数
    keep = 0
    delta_theta_x = 0 #绕x轴旋转的度数
    delta_theta_y = 0 #绕y轴旋转的度数
    delta_theta_z = 0
    delta_beta = 0
    beta_now = 0
    Kb = 10 #beta轴旋转比例系数
    Cb = 0
    Ka = 2
    Torque_z_init = np.zeros(0)
    Torque_z_move = np.zeros(0) #step=2存在移动趋势时收集Rz数据
    Torque_z_keep = np.zeros(0) #step=2且稳定接触平面之后的x方向运动趋势集合
    Torque_z_end = np.zeros(0)
    T_z_init = 0
    T_z_keep = 0 #Rz方向运动趋势的平均值
    T_z_torque = 0
    Torque_z_keep_collect = []
    Torque_z_move_collect = []
    ...
    while (1):
        ...
        message = [keep, delta_theta_x, delta_theta_y, delta_theta_z, beta_now, flash]
        ...
        if len(floats) > 0:
            ...
            if step == 1 and F_z >= 4: #稳定接触的z运动趋势大小可以调整
                ...
                delta_theta_y = 0
                beta_now = 0
                Torque_z_init = np.append(Torque_z_init, T_z)
            ...
            if step == 2:
                ...
                if end1 == 0 and flag == 1: #收集法向运动中的Rz数据，分析其趋势
                    Torque_z_move = np.append(Torque_z_move, T_z)
                if end1 == 1 and flag == 1: #收集稳定后的Rz数据
                    Torque_z_keep = np.append(Torque_z_keep, T_z)
                if end2 == 1 and len(Torque_z_keep) > 7 and flag == 1:
                    flag = 0
                    T_z_init = np.mean(Torque_z_init[1:len(Torque_z_init)-1])
                    T_z_keep = np.mean(Torque_z_keep[1:len(Torque_z_keep)-1])
                    # if abs(T_z_keep) <= 0.1: #当Rz小于阈值时，不再进一步调整
                    #     Threshold = True

                if T_z_keep != 0 and Threshold == False:
                    flag = 1
                    keep = 0
                    delta_beta = Kb * (T_z_keep)
                    beta_now = beta + delta_beta
                    theta_x_now, theta_y_now = beta_to_xy(beta_now, alpha=10)
                    theta_x, theta_y = beta_to_xy(beta, alpha=10)
                    delta_theta_x = theta_x_now - theta_x
                    delta_theta_y = theta_y_now - theta_y

                    flash = 1
                    Torque_z_move_collect.append(Torque_z_move)
                    Torque_z_keep_collect.append(Torque_z_keep)
                    Torque_z_keep = np.zeros(0)
                    Torque_z_move = np.zeros(0)
            ...
            if step == 3: #横向移动判断约束
                ...
                if flash == 0:
                    Threshold = False
                if np.mean(Force_z[-5:-1]) < 2:
                    Threshold = True
                    flash = 1

                if end1 == 1 and flag == 1: #计算旋转趋势的平均值
                    Torque_z_end = np.append(Torque_z_end, T_z)
                if end2 == 1 and len(Torque_z_end) > 5 and flag == 1:
                    flag = 0
                    T_z_torque = np.mean(Torque_z_end[1:len(Torque_z_end)-1])

                count = floats[19]  # 临时这样，后面删去此段代码，改成使用运动趋势判断
            ...
            if step == 4:
                ...
                if T_z_keep != 0 and Threshold == False:
                    ...
                    delta_theta_x = 0
                    delta_theta_y = 0
                    delta_theta_z = Ka * (T_z_keep)
                    beta_now = 0
                    flash = 1
                    # Torque_z_end = np.append(Torque_z_end, T_z_keep)
                    count = floats[19]  # 临时这样，后面删去此段代码
                    if count >= 4:
                        Threshold = True
                elif Threshold == True:
