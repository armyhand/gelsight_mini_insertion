...
if __name__ == "__main__":
    ...
    while (1):
        ...
        message = [keep, delta_theta_x, delta_theta_y, alpha]
        ...
        if len(floats) > 0:
            step = floats[14]
            if step == 1 and F_z >= 3: #稳定接触的z运动趋势大小可以调整
                F_x_keep = 0
                F_y_keep = 0
            ...
            #     flag = 1 # 数组已被清零
            #     alpha_x = floats[15] #收到的是角度值
            #     alpha_y = floats[16]
            #     keep = 0
            #     if F_x_keep != 0 and F_y_keep != 0: #根据比例控制计算角度调整量
            #         delta_theta_x = Kx * (0-F_x_keep)
            #         delta_theta_y = Ky * (0-F_y_keep)
            #         _, alpha = xy_to_beta(alpha_x+delta_theta_x, alpha_y+delta_theta_y)
            #     else:
            #         print("step=2,error")
            #         break
        else:
