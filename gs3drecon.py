...
class Reconstruction3D:
    ...
    def get_depthmap(self, frame, mask_markers, cm=None):
        ...
        # dm, f_env = poisson_reconstruct(gx_interp, gy_interp, boundary)
...
class Visualize3D:
    ...
    def more_construct(self, dm, f, gx, gy, imgh, imgw):
        x = np.arange(self.n)  # * mmpp
        y = np.arange(self.m)  # * mmpp
        XX, YY = np.meshgrid(x, y)

        dn = 1 / 500
        X = np.linspace(-dn * imgh / 2, dn * imgh / 2, imgh)
        Y = np.linspace(-dn * imgw / 2, dn * imgw / 2, imgw)

        fig = plt.figure(1)
        ax = fig.add_subplot(111, projection='3d')
        ax.plot_surface(XX, YY, dm, cmap=cm.coolwarm, rcount=201, ccount=201)
        ...
        # gx = gx.reshape(imgh, imgw)
        fig1, ax1 = plt.subplots()
        pc1 = plt.pcolormesh(X, Y, gx, shading='auto')
        ax1.axis('equal')
        ax1.set(xlim=(-0.5, 0.5), ylim=(-0.5, 0.5))
        fig1.colorbar(pc1)
        plt.show()
        #
        # gy = gy.reshape(imgh, imgw)
        fig2, ax2 = plt.subplots()
        pc2 = plt.pcolormesh(X, Y, gy, shading='auto')
        ax2.axis('equal')
        ax2.set(xlim=(-0.5, 0.5), ylim=(-0.5, 0.5))
        fig2.colorbar(pc2)
        plt.show()

        open3d.visualization.draw_geometries([self.pcd, open3d.geometry.TriangleMesh.create_coordinate_frame(1)])
