...
class Flow:
    ...
    def cal_gripper_force(self, flow_left, flow_right, delta):
        ...
        # 计算夹爪受力情况(机械臂坐标系)
        F_y = -(force_x_left + force_x_right) * 1
        F_x = -(curl_left - curl_right) / 2
        F_z = (force_x_left - force_x_right) / 2
        T_z = (force_y_left - force_y_right) * 1
...
if __name__ == "__main__":
    ...
    cap = cv2.VideoCapture(
        "luoshuan/xuanning_2_success/flow_init picture/flow_init.avi")  # choose to read from video or camera

    # 第二个传感器
    # cap_2 = cv2.VideoCapture(2)
    cap_2 = cv2.VideoCapture("luoshuan/xuanning_2_success/flow_init picture/flow_init_2.avi")
    ...
    floats = np.loadtxt(r'luoshuan/xuanning_2_success/force_bytes.txt')
    Time = np.loadtxt
    Time = np.array(Time)
    Time = Time - Time[0]
    cur_x = floats[:, 21]
    ...
    cur_z = floats[:, 23]
    cur_x = list(cur_x)
    cur_x_ex = cur_x + [cur_x[-1]] * (len(Force_x) - len(cur_x))
    cur_x_ex = np.array(cur_x_ex)
    cur_y = list(cur_y)
    cur_y_ex = cur_y + [cur_y[-1]] * (len(Force_y) - len(cur_y))
    cur_y_ex = np.array(cur_y_ex)
    cur_z = list(cur_z)
    cur_z_ex = cur_z + [cur_z[-1]] * (len(Force_z) - len(cur_z))
    cur_z_ex = np.array(cur_z_ex)
    plt.figure()
    plt.subplot(6, 1, 1)
    plt.plot(Time, cur_x_ex, linewidth=2)
    plt.ylabel('x (mm)', fontsize=18, fontdict={'family': 'Times New Roman'})
    plt.xticks(fontsize=1, fontproperties='Times New Roman')
    plt.yticks(fontsize=18, fontproperties='Times New Roman')
    plt.subplot(6, 1, 2)
    plt.plot(Time, cur_y_ex, linewidth=2)
    plt.ylabel('y (mm)', fontsize=18, fontdict={'family': 'Times New Roman'})
    plt.xticks(fontsize=1, fontproperties='Times New Roman')
    plt.yticks(fontsize=18, fontproperties='Times New Roman')
    plt.subplot(6, 1, 3)
    plt.plot(Time, cur_z_ex, linewidth=2)
    plt.ylabel('z (mm)', fontsize=18, fontdict={'family': 'Times New Roman'})
    plt.xticks(fontsize=1, fontproperties='Times New Roman')
    plt.yticks(fontsize=18, fontproperties='Times New Roman')
    plt.subplot(6, 1, 4)
    plt.plot(Time, Force_x, linewidth=2)
    plt.ylabel('Tx', fontsize=18, fontdict={'family': 'Times New Roman'})
    plt.xticks(fontsize=1, fontproperties='Times New Roman')
    plt.yticks(fontsize=18, fontproperties='Times New Roman')
    plt.subplot(6, 1, 5)
    plt.plot(Time, Force_y, linewidth=2)
    plt.ylabel('Ty', fontsize=18, fontdict={'family': 'Times New Roman'})
    plt.xticks(fontsize=1, fontproperties='Times New Roman')
    plt.yticks(fontsize=18, fontproperties='Times New Roman')
    plt.subplot(6, 1, 6)
    plt.plot(Time, Force_z, linewidth=2)
    plt.xticks(fontsize=18, fontproperties='Times New Roman')
    plt.yticks(fontsize=18, fontproperties='Times New Roman')
    plt.xlabel('time (s)', fontsize=20, fontdict={'family': 'Times New Roman'})
    plt.ylabel('Tz', fontsize=18, fontdict={'family': 'Times New Roman'})
    plt.show()

    ##填充颜色
    plt.subplot(6, 1, 6)
    plt.axvspan(0, 18, facecolor='#00ffff')
    plt.axvspan(18, 36, facecolor='#aaff00')
    plt.axvspan(36, 58, facecolor='#aaaaff')
    plt.axvspan(58, 84, facecolor='#00ffff')
    plt.axvspan(84, 96, facecolor='#aaff00')
    plt.axvspan(96, 190, facecolor='#ffff00')