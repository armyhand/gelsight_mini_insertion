...
if __name__ == "__main__":
    ...
    F_x_keep_c = []
    F_y_keep_c = []
    Force_x_rotation = np.zeros(0) #step=3旋拧过程中的x方向运动趋势集合
    Force_y_rotation = np.zeros(0)  # step=3旋拧过程中的y方向运动趋势集合
    Force_z_rotation = np.zeros(0) #step=3旋拧过程中的y方向运动趋势集合
    Torque_z_rotation = np.zeros(0)  # step=3旋拧过程中的y方向运动趋势集合
    F_x_rotation, F_y_rotation, F_z_rotation, R_z_rotation = 0, 0, 0, 0
    keep = 0 #
    finished = 0
    flag = 1 #标志，为1代表可以写入，为0代表不能写入
    send_flag = 1 #发送与接收标志，为1代表可以通信，为0代表不能通信
    Time = []
    ...
    while (1):
        ...
        message = [keep, delta_x, delta_y, finished]
        ...
        if len(floats) > 0:
            ...
            if step == 0 and F_z <= 3:
                ...
                delta_x = 0
                delta_y = 0
            if step == 0 and F_z >= 3:  # 稳定接触的z运动趋势大小可以调整
                F_x_keep = 0
                F_y_keep = 0
            ...
            if step == 1:
                if len(Force_x_keep) > 0 and len(Force_y_keep) > 0 and flag == 1:  # 计算横向移动趋势的平均值
                    F_x_keep = np.mean(Force_x_keep[1:len(Force_x_keep) - 1])
                    F_y_keep = np.mean(Force_y_keep[1:len(Force_y_keep) - 1])
                    flag = 0  # 数组内含有数据
                    if abs(F_x_keep) <= 0.01 and abs(F_y_keep) <= 0.8:
                        finished = 1.0
                    F_x_keep_c.append(F_x_keep)
                    F_y_keep_c.append(F_y_keep)
                if len(Force_x_keep) == 0 and len(Force_y_keep) == 0 and flag == 1:
                    print("step=1 error")
                    break
                if F_z < -2:
                    finished = 1.0
            ...
            if step == 2:
                Force_x_keep = np.zeros(0)
                Force_y_keep = np.zeros(0)
                flag = 1  # 数组已被清零

                if F_x_keep != 0 and F_y_keep != 0:  # 根据比例控制计算位移调整量
                    if abs(F_x_keep) > 0.025:
                        Kx = 400
                    elif 0.015 <= abs(F_x_keep) <= 0.025:
                        Kx = 300
                    else:
                        Kx = 200
                    if abs(F_y_keep) > 2.5:
                        Ky = 1.3
                    elif 1.5 <= abs(F_y_keep) <= 2.5:
                        Ky = 1
                    else:
                        Ky = 0.75
                    delta_x = -Kx * (F_x_keep)
            ...
            if step == 3 and F_z <= 8:
                ...
                R_z_rotation = 0
                if flag == 1:  # 数组已被清零，可以填充数据
                    Force_x_rotation = np.append(Force_x_rotation, F_x)
                    Force_y_rotation = np.append(Force_y_rotation, F_y)
                    Force_z_rotation = np.append(Force_z_rotation, F_z)
                    Torque_z_rotation = np.append(Torque_z_rotation, T_z)
                else:
                    print("step=3 online")

            if F_z > 10:
                print("the force is too high!")
                break
        ...
        if len(A) > 1:
            A1 = np.array(A)
            np.savetxt(r'force_bytes.txt', A1)
            np.savetxt(r'time.txt', Time)
